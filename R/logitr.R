...
#' |`useAnalyticGrad`|Set to `FALSE` to use numerically approximated gradients instead of analytic gradients during estimation (which is slower). For now, this only applied to MNL models and is ignored for mixed logit models as the numerical approximation is currently faster.|`TRUE`|

...
#' |:---------------|:------------------|:--------------|
#' |`message`|Set to `FALSE` to not display any messages during model
#' estimation.|`TRUE`|
#' |`numMultiStarts`|Number of times to run the optimization loop, each time
#' starting from a different random starting point for each parameter between
#' `startParBounds`. Recommended for non-convex models, such as WTP space models
#' and MXL models.|`1`|
#' |`keepAllRuns`|Set to `TRUE` to keep all the model information for each
#' multistart run. If `TRUE`, the `logitr()` function will return a list with
#' two values: `models` (a list of each model), and `bestModel` (the model with
#' the largest log-likelihood value).|`FALSE`|
#' |`startParBounds`|Set the `lower` and `upper` bounds for the starting
#' parameters for each optimization run, which are generated by
#' `runif(n, lower, upper)`.|`c(-1, 1)`|
#' |`startVals`|A vector of values to be used as starting values for the
#' optimization. Only used for the first run if `numMultiStarts > 1`.|`NULL`|
#' |`useAnalyticGrad`|Set to `FALSE` to use numerically approximated gradients
#' instead of analytic gradients during estimation (which is slower).|`TRUE`|
#' |`scaleInputs`|By default each variable in `data` is scaled to be between 0
#' and 1 before running the optimization routine because it usually helps with
#' stability, especially if some of the variables have very large or very small
#' values (e.g. `> 10^3` or `< 10^-3`). Set to `FALSE` to turn this feature
#' off.|`TRUE`|
...
#' matrix of standard normal draws if desired.|`NULL`|
#' |`numDraws`|The number of draws to use for MXL models for the maximum
#' simulated likelihood.|`50`|
#' |`printLevel`|The print level of the `nloptr` optimization loop. Use
#' `nloptr::nloptr.print.options()` for more details.|`0`|
#' |`xtol_rel`|The relative `x` tolerance for the `nloptr` optimization loop.
#' Use `nloptr::nloptr.print.options()` for more details.|`1.0e-6`|
#' |`xtol_abs`|The absolute `x` tolerance for the `nloptr` optimization loop.
#' Use `nloptr::nloptr.print.options()` for more details.|`1.0e-6`|
#' |`ftol_rel`|The relative `f` tolerance for the `nloptr` optimization loop.
#' Use `nloptr::nloptr.print.options()` for more details.|`1.0e-6`|
#' |`ftol_abs`|The absolute `f` tolerance for the `nloptr` optimization loop.
#' Use `nloptr::nloptr.print.options()` for more details.|`1.0e-6`|
#' |`maxeval`|The maximum number of function evaluations for the `nloptr`
#' optimization loop. Use `nloptr::nloptr.print.options()` for more details.
#' |`1000`|
#'
#' @return
...
#' |`message`|A more informative message with the status of the optimization
#' result.|
#' |`status`|An integer value with the status of the optimization (positive
#' values are successes). Use [statusCodes()] for a detailed description.|
#' |`modelSpace`|The model space (`'pref'` or `'wtp'`).|
#' |`standardDraws`|The draws used during maximum simulated likelihood (for
#' MXL models).|
#' |`randParSummary`|A summary of any random parameters (for MXL models).|
#' |`parSetup`|A summary of the distributional assumptions on each model
#' parameter (`"f"`="fixed", `"n"`="normal distribution", `"ln"`="log-normal
#' distribution").|
#' |`options`|A list of all the model options.|
#' |`multistartSummary`|A summary of the log-likelihood values for each
#' multistart run.|
#'
#' @export
#' @examples

...
distributions. It is typically about 3-4 times faster than the native R
implementation.
...
By default (`numThreads = NULL`), all available cores are used, *except* when
running a parallel multistart (`numMultiStarts > 1`): in that case a single
...
- **Most users need to do nothing.** Mixed logit models already use the compiled
  `"cpp"` backend across all available cores by default, and it handles large
  draw counts without running out of memory.

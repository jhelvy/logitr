...
Estimated models can be used to predict expected choices and choice probabilities for a set (or multiple sets) of alternatives based on an estimated model. As an example, consider predicting choices and probabilities for two of the choice observations from the `yogurt` dataset:
...
In the example below, the expected choice probabilities for these sets of alternatives are computed using the fixed parameter `mnl_pref` model:
...
The `probs` data frame contains the expected choice probabilities for each alternative. The low and high values show a 95% confidence interval, estimated using the Krinsky and Robb parametric bootstrapping method [@Krinsky1986]. You can change the CI level by setting alpha to a different value (e.g. a 90% CI is obtained with alpha = 0.05).

Likewise, expected choices can also be predicted for these alternatives using the same `mnl_pref` model. The optional `choiceName` argument defines which column contains the actual choices made (for example, if predicting choices on a hold out sample):
...
Both `predictProbs()` and `predictChoices()` work with models estimated in either preference or WTP space utility models.

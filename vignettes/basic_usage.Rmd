...
The `data.frame` provided to the `data` argument must be arranged in a particular way (i.e. "long" format). Each row should be an alternative from a choice observation, and the choice observations do not have to be symmetric (i.e. each choice observation could have a different number of alternatives). The data must include columns for each of the following arguments in the `logitr()` function:

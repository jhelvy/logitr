...
accuracy <- round(100*sum(chosen$correct) / nrow(chosen), 1)
```

The `choices` data frame contains the actual choices, `choice`, and the predicted choices, `choice_predict`. In this case, the model accurately predicted the actual choice in `r accuracy`% of the choice observations:

...
```{r, child='rmdchunks/header.Rmd'}
```

## Data format

```{r, child='rmdchunks/dataFormat.Rmd'}
```
...
```{r, child='rmdchunks/yogurtUtilityPref.Rmd'}
```
...
```{r, child='rmdchunks/yogurtUtilityWtp.Rmd'}
```
...
WTP space models can also be used to predict probabilities, but the `price` argument must be specified for predictions being made on a new data set:

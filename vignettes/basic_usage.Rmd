...
```{r, eval=FALSE}
shares <- predictShares(mnl_pref, alts)
shares
```

Likewise, you can use the `predictChoices()` function to predict choices for a set of alternatives:

```{r, eval=FALSE}
choices <- predictChoices(mnl_pref, alts)
choices
```

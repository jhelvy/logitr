...
## Predict shares and choices

Once you have estimated a model, you can use it to predict the expected choices or shares of a particular set of alternatives (or multiple sets of alternatives). To predict shares, use the `predictShares()` function using an estimated model and a data frame of alternatives. As an example, I'll use a couple of the choice observations from the `yogurt` dataset (note that if you include more than one set of alternatives you also have to specify an `obsID`):
...
Likewise, you can use the `predictChoices()` function to predict choices for a set of alternatives. The optional `choiceName` argument defines which column contains the actual choices made:

...
where $\betaVec$ is the vector of coefficients for non-price attributes $\xVec_{j}$, $\alpha$ is the coefficient for price $p_{j}$. Accordingly, each parameter in the `pars` argument is an additive part of $v_{j}$.

For example, if the observed utility for yogurt purchases was $v_{j} = \beta x_{j} + \alpha p_{j}$, where $p_{j}$ is "price" and $x_{j}$ is "brand", then the `pars` argument should be `c("price", "brand")`. This model can be estimated as follows using the `yogurt` data frame:
...
For models estimated in the WTP space, the `modelSpace` argument should be set to `"wtp"`. In addition, the `pars` argument should _not_ contain the name of the column for the price attribute because there is no estimated "price" parameter in WTP space models. Instead, a "scale" parameter given by $\lambda$ will be estimated, and the column for "price" should be provided separately using the `price` argument.

For example, if the observed utility was $v_{j} = \lambda \left(\omega x_{j} - p_{j}\right)$, where $p_{j}$ is "price" and $x_{j}$ is "brand", then the `pars` argument should be `"brand"` and the `price` argument should be `"price"`. This model can be estimated as follows using the `yogurt` data frame:

...
If you include a factor or character type variable in a model, it will by default be modeled as a discrete variable, and the default reference level is usually the alphabetically first level. For example, in the `yogurt` data, the `brand` variable has four levels: `"dannon"`, `"weight"`, `"hiland"`, and `"yoplait"`. Since `"dannon"` is alphabetically first, it will be set as the reference level:
...
The `dummyCode()` function can be used to create dummy-coded variables from a discrete variable. In the code below, I create dummy-coded columns for the `brand` variable and then use those variables as covariates in a model: 

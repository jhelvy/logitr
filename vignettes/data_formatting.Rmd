...
For the first option, just convert the column in the data frame to a `"character"` or `"factor"` type prior to estimating the model. For example, consider the following model:
...
In this model, since the `price` variable is a `"double"` variable type, it is modeled as a continuous variable by default:
...
The new `cars_us_dummy` data frame now contains new variables for each level of the `price` column. This approach allows the modeler to specify the baselne level. In this example, I'll use the `price_50` level as the baseline:

...
2. Each row is an alternative from a choice observation. Choice observations do not have to be symmetric (i.e. they could each have a different number of alternatives).
3. Each column is a variable. 
4. One column must identify the "observation ID" --- a sequence of numbers that identifies each unique choice occasion. For example, if the first three choice occasions had 2 alternatives each, then the first 9 rows of the `obsID` variable would be `1,1,2,2,3,3`.

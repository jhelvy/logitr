...
A structural assumption in the `logitr` package is that the deterministic part of the utility specification is linear in parameters: $v_{j} = \betaVec' \xVec_{j}$ for preference space models, and $v_{j} = \omegaVec' \xVec_{j}$ for WTP space models. Accordingly, each parameter in the `parNames` argument is an additive part of $v_{j}$. For example, for the utility model $u_{j} = \beta_1 price + \beta_2 size + \varepsilon_{j}$, then the `parNames` argument should be `c('price', 'size')`, and the data should contain two columns labeled `price` and `size`. If you wanted to add a third parameter, say $price^2$, then you should create an additional variable in the data called `priceSquared`, and your `parNames` argument would be `c('price', 'size', 'priceSquared')`.
    
**WTP space models**:
For WTP space models, the `parNames` should only include the WTP parameters, and the `price` parameter is denoted by the separate argument `priceName`. For example, for the utility model *u* = *lambda*(*beta1* * *size* - *price*) + *error*, then the `parNames` argument should be `c('size')` and the `priceName` argument should be `'price'
...
```
# WARNING:
**This document is not complete and contains many errors**
...
```{r}
mnl.pref.simulation = marketSimulation(mnl.pref, market, 
                                       alpha=0.025)
mnl.pref.simulation
```
...
```{r}
mnl.wtp.simulation = marketSimulation(mnl.wtp, market,
                                      priceName="price", alpha=0.025)
mnl.wtp.simulation
```

**Run the simulation using the preference space MXL model**:
```{r eval=FALSE}
mxl.pref.simulation = marketSimulation(mxl.pref, market, 
                                       alpha=0.025)
```
```{r}
mxl.pref.simulation
...
```{r eval=FALSE}
mxl.wtp.simulation = marketSimulation(mxl.wtp, market,
                                      priceName="price", alpha=0.025)
```
```{r}
mxl.wtp.simulation

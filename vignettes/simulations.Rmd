...
vignette: >
  %\VignetteIndexEntry{Simulating shares}
...
Once a model has been estimated, it can be used to simulate the predicted shares for a set of alternatives. To do so, you first need to create a set of alternatives where each row is an alternative and each column an attribute. In this example, I'll just use one of the choice observations from the `yogurt` dataset:
```{r}

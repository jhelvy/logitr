...
You can also use WTP space models to predict choice, but you must provide the additional `priceName` argument to the `predictChoice()` function. For example, here are the results from an equivalent model but in the WTP space:
...
You can also use mixed logit models to make predictions. Heterogeneity is modeled by simulating draws from the population estimates of the estimated model. Here is an example using a preference space mixed logit model:

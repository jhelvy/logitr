...
You can also obtain the upper and lower bounds of a confidence interval for predicted probabilities by including the `ci` argument, which are estimated using the Krinsky and Robb parametric bootstrapping method [@Krinsky1986]. For example, a 95% CI is obtained with `ci = 0.95`:
...
WTP space models estimated using `logitr()` can also be used to predict probabilities. The only additional requirement is that if predictions are being made on a different data set using the `newdata` argument, you must also specify the `price` argument to identify which column in `newdata` corresponds to "price". In the example below, a WTP space model is estimated and used to predict probabilities for the same `data` data frame as in the previous examples:
...
You can also use the `predict()` method to predict outcomes by setting `type = "outcome"` (the default value is `"prob"` for predicting probabilities). If no new data are provided for the `newdata` argument, then outcomes will be predicted for every alternative in the original data used to estimate the model.

In the examples below the `returnData` argument is also set to `TRUE` so that the predicted outcomes can be compared to the actual ones. Here are two sets of predictions, one using the estimated preference space model and the other using the WTP space model. The predicted outcomes may vary across different iterations of the same code since they are chosen according to the predicted probabilities, and thus there is a chance other outcomes may be made.

```{r}
outcomes_mnl_pref <- predict(mnl_pref, type = "outcome", returnData = TRUE)
head(outcomes_mnl_pref)

outcomes_mnl_wtp <- predict(mnl_wtp, type = "outcome", returnData = TRUE)
head(outcomes_mnl_wtp)
```
...
```{r}
chosen_pref <- subset(outcomes_mnl_pref, choice == 1)
...
chosen_wtp <- subset(outcomes_mnl_wtp, choice == 1)

...
- Added the `numDrawsBatch` argument to stream the random draws in batches when evaluating the mixed logit log-likelihood. This keeps peak memory bounded by the batch size rather than the total number of draws, which makes large draw counts (e.g. 10,000+) feasible. By default logitr streams automatically only when the draws would otherwise exceed an internal memory budget, so typical models are unaffected.
